...
public class PositionalGame extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        canvas.setOnMouseClicked(event -> {
            ...
            if(!playWithComputer) {
                ...
                if(selectedLine != null && !selectedLine.isColored(canvas)) {
                    selectedLine.colorLine(canvas, isRedTurn);
                    if(!graph.get().checkWin(isRedTurn, canvas)) {
                        gameAI.get().addOpponentLine(selectedLine);
                        selectedLine = gameAI.get().getBestMove();
                        selectedLine.colorLine(canvas, !isRedTurn);
                        graph.get().checkWin(!isRedTurn, canvas);
                    }
                }
        ...
        // Control panel
        Button playWithComputerButton = new Button("Play with Computer");
        playWithComputerButton.setOnAction(event -> {
            if(!playWithComputer) {
                playWithComputer = true;
                if(gameAI.get() == null) {
                    gameAI.set(new GameAI(graph, isRedTurn));
                }
            }
        });
        ...
        HBox controlPanel  = new HBox(playWithComputerButton, loadButton, saveButton, resetButton, exitButton);
    ...
    private void newGame(Canvas canvas, Graph graph, AtomicReference<GameAI> gameAI) {
